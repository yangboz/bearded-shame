...
import info.smartkit.jreport.dto.JsonString;
import info.smartkit.jreport.dto.JreportInfo;
import info.smartkit.jreport.utils.FileUtil;
import info.smartkit.jreport.utils.JreportInfoHelper;
...
public class FileUploadController {
	...
	@RequestMapping(method = RequestMethod.POST, value = "/info/smartkit/jreport/file/upload", consumes = MediaType.MULTIPART_FORM_DATA)
	...
	public @ResponseBody JsonObject handleSingleFileUpload(
		...
		JreportInfo jReportInfo = new JreportInfo();
		if (!file.isEmpty()) {
			...
			try {
				...
				LOG.info("ImageMagick output success: " + fileName);
				 String imageUrl = JreportInfoHelper.getRemoteImageUrl(fileName);
				// JReporting:
			} catch (Exception ex) {
		...
		return new JsonObject(jReportInfo);
	}
